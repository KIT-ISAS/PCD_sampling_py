...
class PCDSamplingStrategy:
    ...
    def __init__(self, config: PCDSamplingConfig):
        ...
        :param K: number of unit vectors
        """
        ...
        self._delta_r_vmap = torch.func.vmap(self.calculate_delta_r, in_dims=(0, None, None, None, None))
        self._delta_x_vmap = torch.func.vmap(self.calculate_delta_x, in_dims=(0, 0, 0, 0, None))
        self._delta_r_sorted_vmap = torch.func.vmap(self.calculate_delta_r_sorted, in_dims=(0, 0, None, None, None))
        self._delta_x_sorted_vmap = torch.func.vmap(self.calculate_delta_x_sorted, in_dims=(0, 0, 0, 0, None))
    ...
    def create_unit_vectors_random(self):
        ...
        self.unit_vectors = torch.distributions.uniform.Uniform(-1.0, 1.0).sample((self.number_unit_vectors, self.dim))
        self.unit_vectors = self.unit_vectors / self.unit_vectors.norm(dim=1, keepdim=True)

    def create_unit_vectors_deterministic(self): ...

    @torch.compile
    def calculate_delta_r(self, x: Tensor, r: Tensor, weights: Tensor, means: Tensor, stds: Tensor):
        """
    ...
    @torch.compile
    def calculate_delta_r_sorted(self, x: Tensor, i: Tensor, weights: Tensor, means: Tensor, stds: Tensor):
        """
        ...
        eps = 1e-3  # to avoid division by zero.
        return -((F_tilda - (2 * i - 1) / 2 / self.number_samples) / torch.clamp(f_tilda, min=eps))

    @torch.compile
    def calculate_delta_x(self, r: Tensor, u: Tensor, means: Tensor, stds: Tensor, weights: Tensor):
        """
    ...
    @torch.compile
    def calculate_delta_x_sorted(self, r: Tensor, u: Tensor, means: Tensor, stds: Tensor, weights: Tensor):
        """
        This function is used to compute the gain for all samples with respect to the provided unit vector.
        ...
        delta_r_sorted = self._delta_r_sorted_vmap(sorted_r, self.numbers, weights, means, stds)
    ...
    def sample(self, gm: GaussianMixture):
        ...
        for _ in range(self.steps):
            coef = self.alpha_min * _ / self.steps + self.alpha_max * (self.steps - _) / self.steps
            ...
            if torch.norm(delta_x.sum()) < self.threshold:  # works better with sum() and then summing over a vector.
                return X
            X += coef * delta_x
